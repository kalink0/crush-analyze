...
Deliberate duplicate of crush-forensics' own `crush/parsers/abx_decoder.py`
(same author, same license -- not a third-party vendoring case, just this
tool's own already-written, already-debugged code reused in a sibling
project that can't depend on crush-forensics itself). Keep the two in sync
by hand if either gets a bug fix; there is no shared package to import from
